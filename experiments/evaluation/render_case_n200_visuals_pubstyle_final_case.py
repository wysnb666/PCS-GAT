...
# 路径与配置
# =========================
GRAPHS_JSON = "MPC4plus/new_artifacts/final_pathaware_gat_mean_3l/01_graphs/large_zero_shared_v3/graphs.json"
SPLIT_JSON = "MPC4plus/new_artifacts/final_pathaware_gat_mean_3l/01_graphs/large_zero_shared_v3/split.json"
GRAPH_ID = "largezero_er_n200_p0p025_011"

GNN_CHECKPOINT = "MPC4plus/new_artifacts/final_pathaware_gat_mean_3l/03_models/s123/pathgat3/checkpoint.pt"
CPSAT_900S_JSON = ""

OUTPUT_DIR = "MPC4plus/new_artifacts/final_pathaware_gat_mean_3l/06_figures/case_n200_011_pubstyle_netgain"
...
def draw_difference_panel(ax, G: nx.Graph, pos, rule_sol: nx.Graph, gnn_sol: nx.Graph, opt_sol: nx.Graph | None) -> None:
    ...
    if opt_sol is not None:
        ...
        missed_vs_opt = sorted(all_nodes - gnn_nodes)
    ...
    txt = f"Difference view\nRule: {len(rule_nodes)}, PCS-GAT: {len(gnn_nodes)}\nNet gain: {len(gnn_nodes)-len(rule_nodes):+d} vertices\nStill uncovered: {len(missed_vs_opt)}"
    ax.text(
        0.02, 0.98, txt,
        transform=ax.transAxes,
        va="top", ha="left",
...
def main() -> None:
    ...
    rule_sol = sols["rule"]
    gnn_sol = sols["gnn"]
    opt_sol = load_cpsat_opt_solution_graph(CPSAT_900S_JSON) if (CPSAT_900S_JSON and Path(CPSAT_900S_JSON).is_file()) else None
    ...
    fig, ax = plt.subplots(figsize=(10.6, 7.2), facecolor="white")
    draw_solution_panel(ax, G, pos, gnn_sol, "PCS-GAT")
    save_figure(fig, outdir / "case_n200_pathgat3_pubstyle")
